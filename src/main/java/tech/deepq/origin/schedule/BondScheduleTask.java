...
public class BondScheduleTask {
    ...
    @Value("${sync-tasks.bond-table-name}")
    private String bondTableName;
    ...
    public void syncBondData() {
        ...
        sqlRowSet = jdbcTemplate.queryForRowSet("select a.BOND_ID bondId, a.PARTY_ID partyId, c.SEC_SHORT_NAME shortName, c.SEC_FULL_NAME fullName " +
            " from  bond a inner join bond_ticker b on a.BOND_ID = b.BOND_ID " +
            " inner join md_security c on b.SECURITY_ID = c.SECURITY_ID " +
        ...
        while (sqlRowSet.next()) {
            ...
            if (!StringUtils.hasText(bondDTO.getAliasString())) {
                ...
                if (StringUtils.hasText(sqlRowSet.getString("partyId"))) {
                    bondDTO.setRelateCompany("CP_" + sqlRowSet.getString("partyId"));
                } else {
                    bondDTO.setRelateCompany("");
                }
        ...
        for (BondDTO bondDTO : bondAliasMap.values()) {
            logger.info("bond DTO is [{}]", bondDTO);
            if (StringUtils.hasText(bondDTO.getCode())) {
    ...
    private boolean checkBond(BondDTO bondDTO) {
        return secondaryJdbcTemplate.queryForRowSet("select * from " + bondTableName + " where code = '" + bondDTO.getCode() + "'").next();
    }

    private void updateBond(BondDTO bondDTO) {
        try {
            secondaryJdbcTemplate.update("update " + bondTableName + " set name = ?, alias_string = ?, " +
            ...
            logger.error("insertBond exist ERROR: ", e);
        }
    }

    private void insertBond(BondDTO bondDTO) {
        try {
            secondaryJdbcTemplate.update("insert into " + bondTableName + " (id, market_code, " +
